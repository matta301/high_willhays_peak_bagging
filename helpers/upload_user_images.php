...
class Upload_user_images
{
    private $summit_custom_dir = '/summit_images/';
    private $id;
     ...
     *  @return boolean
     */
    function upload_summit_images( $user_id, $uploaded_file, $peak_id )
    {
        $this->id = $user_id;

        // File Data
        $file      = $uploaded_file["peak_summit_image"];
        ...
        $new_file_name = wp_unique_filename( $upload_dir['basedir'] . '/summit_images/user_' . $this->id . '/', $file_name );
        ...
        $file_img_path = $upload_dir["baseurl"] . $this->summit_custom_dir . 'user_' . $this->id . '/' . basename( $new_file_name );

        $attachment = array(
            'guid' => $file_img_path,
            'post_mime_type' => $file_mime,
            'post_title' => $file_name,
            'post_name' => preg_replace( '/\\.[^.\\s]{3,4}$/', '', $file_name ),
            'post_content' => '',
            'post_status' => 'inherit'
        );
        ...
        $attachment_data = wp_generate_attachment_metadata( $attachment_id, $upload_dir["basedir"] . $this->summit_custom_dir . 'user_' . $this->id . '/' . basename( $new_file_name ) );
        ...
        foreach ($attachment_data['sizes'] as $value) {
            $attachment = array(
                'guid' => $upload_dir["baseurl"] . $this->summit_custom_dir . 'user_' . $this->id . '/' . $value["file"],
            ...
            wp_insert_attachment( $attachment, $value["file"], $peak_id );
        }
    ...
    function summit_upload_dir( $dir_data ) {
        return array(
            'path' => $dir_data[ 'basedir' ] . $this->summit_custom_dir . 'user_' . $this->id,
            'url' => $dir_data[ 'baseurl' ] . $this->summit_custom_dir . 'user_' . $this->id,
            'subdir' => '/' . $this->summit_custom_dir,
